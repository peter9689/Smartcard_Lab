...
def perform_dpa(plaintexts, traces, rate):
    ############################# PERFORM DPA ##################################
    ...
    for x in range(TRACES):
        ...
        # DiffTrace[x] = [np.sum(abs(traces[x][y*Shift:y*Shift + RATE])) for y in range(NewSample)]
    ...
    for i in range(BYTE16):
        ...
        # for x in (range(82) + range(83,125) + range(126,KEY256)):
        for x in range(KEY256):
            for y in range(TRACES):
                HammingWeight[y][x] = np.float32(hamming_weight[SBox[plaintexts[y][i] ^ x]]) #get the hamming weight of each probabity of key
        # print HammingWeight
        ...
        print 'power model', i, 'is finished'
