...
static void KeyExpansion_m(void)
{
  ...
  for(i = 0; i < Nk; ++i)
  {
    ...
    for(i = 0; i < 16; ++i)
    {
         if (i % Nk == 0)
        {
            RoundKey_temp[i] = RoundKey[i] ^mask1^mask_sub2;
        }

        else if (i % Nk == 1)
        {
            RoundKey_temp[i] = RoundKey[i] ^mask2^mask_sub2;
        }

        else if (i % Nk == 2)
        {
            RoundKey_temp[i] = RoundKey[i] ^mask3^mask_sub2;
        }
        else
        {
            RoundKey_temp[i] = RoundKey[i] ^mask4^mask_sub2;
        }
    }
    i = Nk;
  // All other round keys are found from the previous round keys.
  for(; (i < (Nb * (Nr + 1))); ++i)
  {
    ...
    RoundKey_temp[i * 4 + 0] = RoundKey[(i - Nk) * 4 + 0] ^ tempa[0] ^mask1^mask_sub2;
    RoundKey_temp[i * 4 + 1] = RoundKey[(i - Nk) * 4 + 1] ^ tempa[1] ^mask2^mask_sub2;
    RoundKey_temp[i * 4 + 2] = RoundKey[(i - Nk) * 4 + 2] ^ tempa[2] ^mask3^mask_sub2;
    RoundKey_temp[i * 4 + 3] = RoundKey[(i - Nk) * 4 + 3] ^ tempa[3] ^mask4^mask_sub2;
  }
  //set the key value with mask
  for(i=0;i<176;i++)
    RoundKey[i] = RoundKey_temp[i];
...
void InvMixColumns(void)
{
  ...
  for(i=0;i<4;++i)
  {
    ...
    (*state)[i][0] = get_times14(a) ^ get_times11(b) ^ get_times13(c) ^ get_times9(d);
    (*state)[i][1] = get_times9(a) ^ get_times14(b) ^ get_times11(c) ^ get_times13(d);
    (*state)[i][2] = get_times13(a) ^ get_times9(b) ^ get_times14(c) ^ get_times11(d);
    ...
    for(j=0;j<4;++j)
     {
		...
		(*state)[seq1[j]][seq2[i]] = getSBoxInvert((*state)[seq1[j]][seq2[i]]);
	  #endif
	  #if (defined (M) && !defined(ALL) && !defined(S))
		(*state)[j][i] = getSBox_mInvert((*state)[j][i]);
	  #endif
	  #if (defined(ALL) || (defined(S) && defined(M)))
		(*state)[seq1[j]][seq2[i]] = getSBox_mInvert((*state)[seq1[j]][seq2[i]]); //maybe we should correct this as well
	  #endif
...
void InvCipher(void)
{
  ...
  // The MixColumns function is not here in the last round.
	...
	AddRoundKey(0);
		...
		for(i=0;i<4;++i)
		{
			(*state)[i][0] = (*state)[i][0]^mask1;
			(*state)[i][1] = (*state)[i][1]^mask2;
			(*state)[i][2] = (*state)[i][2]^mask3;
			(*state)[i][3] = (*state)[i][3]^mask4;
		}
	#endif 
